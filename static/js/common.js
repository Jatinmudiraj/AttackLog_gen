...
// Populate a <select> with prompt template files (scenario prompts)
async function loadPromptTemplates(selectId) {
    const sel = document.getElementById(selectId);
    if (!sel) return;
    ...
    const def = document.createElement("option");
    def.value = "";
    def.textContent = "-- Optional prompt template --";
    sel.appendChild(def);

    try {
        const res = await fetch("/api/list_prompt_templates");
        const files = await res.json();
